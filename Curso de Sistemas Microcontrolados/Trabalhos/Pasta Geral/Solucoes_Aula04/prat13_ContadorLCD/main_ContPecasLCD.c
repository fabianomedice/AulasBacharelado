 ...
 *
 ******************************************************************************
 * Nome do Arquivo:		main_ContPecasLCD.c
 ...
 * Autor:				SEMPRA
 * Versão:				v1.0
 * Descrição:	Prática 13: Contador de Peças no Display LCD
 *		Usar o BOTAO1 para simular um sensor que gera um pulso a cada passagem
 *	de uma peça na linha de montagem e fazer um contador de peças.
 *		O valor da contagem deve ser apresentado no Display LCD (usando
 *	a biblioteca displayLCD.h da SEMPRA.
 *		Use uma técnica de Debounce se necessário.
 *	
 *****************************************************************************/
...
void main(void)
{
	unsigned int cont = 0;	// Armazena o valor da contagem das peças
	...
	// Escreve na linha1 do LCD
	PosicaoCursorLCD(1,1);
	EscreveFraseRomLCD("Cont. Pecas LCD");
	
	//Escreve na linha2 do LCD
	PosicaoCursorLCD(2,1);
	EscreveFraseRomLCD("num: ");
	
	// Laço de repetição infinito que executa aplicação do firmware
	while(1)
	{
		// Se botão 1 foi pressionado, incrementa "contador"
		if(BOTAO1)
		{
			...
			// Espera soltar botão
			while(BOTAO1)	{ /*não faz nada no while, só gasta tempo até soltar botão */ }

		}
		
		//Escreve o valor atual do contador no LCD
